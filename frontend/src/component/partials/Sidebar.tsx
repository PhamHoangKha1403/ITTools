...
function Sidebar({ isVisible }: { isVisible: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Menu */}
      <aside className="w-full overflow-y-auto pl-4 pr-2 pt-4 pb-50 transition-all duration-300 max-h-[calc(100vh-80px)] custom-scroll">
        {menuItems.length === 0 ? (
...
function IconSidebar({ item }: { item: MenuItem }) {
  ...
  <button
    key={childIdx}
    onClick={() => navigate(`/tools/${child.id}`)}
  ...
  >
    <span>{child.title}</span> {/* ✅ Show tên tool thay vì ID */}
  </button>
))}
