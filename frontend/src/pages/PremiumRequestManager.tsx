                                    ...
                                    {req.status === 0 && (
                                        <>
                                            ...
                                            <button
                                                ...
                                                title={`Reject request ${req.id} for user ${req.requestedUser.username}`}
                                            >
                                                Reject
                                            </button>
                                        </>
                                    )}
                                    {req.status !== 0 && (<span className="text-gray-500 text-sm inline-block w-20 text-center">—</span>)}

                                    <button
                                        className="text-gray-400 hover:text-gray-300 hover:underline transition text-sm font-medium ml-2 disabled:opacity-50 disabled:cursor-not-allowed"
                                        onClick={() => handleDelete(req.id, req.requestedUser.id)}
                                        title={`Permanently delete request ${req.id}`}
                                    >
                                        Delete
                                    </button>
                                </td>
